from org_djistras import Graph

class plan_algo:

    def __init__(self, strn, rack, dock):
        self.strn = strn
        self.rack = rack
        self.dock = dock
    ...
    def plan_algo(self):
        ...
        r1 = self.rack #'rack 07'  target rack

        def short_route_rack_to_dock(r,d):                           # finding the shortest path to a dock
            llr1 = graph.dijkstra(self.strn,rack[r][0]) # define starting point here and ending point and rack
            llr2 = graph.dijkstra(self.strn,rack[r][1])
        ...
        ll = short_route_rack_to_dock(r1, d1)
        ...
        qr_data = []
        for i in range(len(ll)-1):
            qr_data.insert(i, Ver[ll[i]])
            print(Ver[ll[i]][0], Ver[ll[i]][1])

        print('rack qr code',qr_data[pointer][0], qr_data[pointer][1])
        ...
        #    delta y is positive then global north
            if (delta_x <= 1)and(delta_x >= -1)and(delta_y > 0):
                actions.insert(i, 0)
        #   delta x is positive then global east
            elif(delta_x > 0)and(delta_y <= 1)and(delta_y >= -1):
                actions.insert(i, 1)
        #   delta y is negative then global south
            elif(delta_x <= 1)and(delta_x >= -1)and(delta_y < 0):
                actions.insert(i, 2)
        ...
        #Send this compass at rack to get arrow
            if i == pointer:
                #compass1 = compass
                for j in range(len(compass)):
                    if(compass[j] == 'N'):
                        ar = j     # store the index of local North
                        #print("arrow", ar)
                #print("here rack", i)
